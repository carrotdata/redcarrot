...
/**
 * Simple user session class User Session structure: "SessionID" - A unique, universal identifier
 * for the session data structure (16 bytes). "Host" - host name or IP Address The location from
 * which the client (browser) is making the request. "UserID" - Set to the user's distinguished name
 * (DN) or the application's principal name. "Type" - USER or APPLICATION "State" - session state:
 * VALID, INVALID Defines whether the session is valid or invalid. "MaxIdleTime" - Maximum Idle Time
 ...
 * information. "StartTime" - start session time (timestamp - 8 bytes) "LastActiveTime" - last
 * interaction time (timestamp - 8 bytes)
 */
