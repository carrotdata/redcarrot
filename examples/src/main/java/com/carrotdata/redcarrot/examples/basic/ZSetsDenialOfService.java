 ...
 * <p>
 * {host_name, number_of_requests} pairs in a sorted sets to track most offensive sites (for TopN
 ...
 * <p>
 * number_of_requests is random number between 1 and 1000 heavily skewed towards 0 host - synthetic
 * (xx.xx.xx.xx) string or a real host name from a provided file
 * <p>
 * Results: 0. Average data size {host, number_of_requests} = 21 bytes 1. No compression. Used RAM
 * per session object is 53 bytes 2. LZ4 compression. Used RAM per session object is 34 bytes 3.
 * LZ4HC compression. Used RAM per session object is 30 bytes, ZSTD - 18 bytes
 * <p>
 * Redis usage per record , using ZSets is 112
 * <p>
 * Total used Redis memory is shown by 'used_memory' in Redis CLI.
 * <p>
 * RAM usage (Redis-to-Carrot)
 * <p>
 * 1) No compression 112/53 ~ 2.1x 2) LZ4 compression 112/34 ~ 3.3x 3) LZ4HC compression 112/30 =
 * 3.8x, 4) ZSTD = 112 / 18 ~ 6.4
 * <p>
 * Effect of a compression:
 * <p>
 * LZ4 - 53/34 ~ 1.6x (to no compression) LZ4HC - 53/30 ~ 1.8x (to no compression), ZSTD = 53 / 18
 * =~ 3.0x
 */
public class ZSetsDenialOfService {
  ...
  /*
   * We use low maximum request count to reflect the fact that majority of hosts are legitimate and
   * do not overwhelm the service wit a bogus requests.
   */
  ...
  private static void runTest() throws IOException, OperationFailedException {
    ...
    log.debug(
      ...
      num, totalDataSize, endTime - startTime, UnsafeAccess.getAllocatedMemory(),
