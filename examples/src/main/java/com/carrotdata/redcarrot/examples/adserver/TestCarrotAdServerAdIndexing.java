 ...
 * <p>
 * 13. SiteAds: ZSET - ordered set. key = siteId, member = adId, score = CTR (click through rate).
 * This data allows us to estimate how does the ad perform on a particular site relative to other
 * ads.
 * <p>
 * 14. SiteWords: ZSET - ordered set. key = siteId, member = word, score - word's value. This data
 * store keeps keywords with corresponding scores. Every time someone acts on ads on the site, all
 * keywords from the ad are added to the site's ordered set with a some score value. The more a
 ...
 * <p>
 * What is the 'word' in this application? Word is not only keyword of an add, such as "car",
 * "luxury", "lease", but also can be a targeting attribute, such as "@sex=female", "@age=20-30",
 * "@income=100K+" etc.
 * <p>
 * Notes (assumptions for our tests):
 * <p>
 * 1. Number of ads is 10K 2. Number of words we estimate as 10K 3. Number of locations we estimate
 * as 10K 4. Number of users of the ad targeting platforms we estimate as 10M 5. Number of sites is
 * 10K.
 * <p>
 ...
 * -- Ads indexing
 * <p>
 * 1. Redis 7.2.4 - 725,312,048 2. Carrot no compression - 179,609,408 3. Carrot LZ4 compression -
 * 83,026,176 4. Carrot ZSTD compression - 56,552,128 Memory ratio Redis/Carrot: 1. Carrot no
 * compression ~ 4.0x 2. Carrot LZ4 ~ 8.7x 3. Carrot ZSTD ~ 12.8x
 * <p>
 * -- User targeting 1. Redis 7.2.4 = 994,670,544 2. Carrot no compression = 255,629,376 3. Carrot
 * LZ4 compression = 223,004,224 4. Carrot ZSTD compression = 203,970,624 Memory ratio Redis/Carrot:
 * 1. Carrot no compression ~ 3.9x 2. Carrot LZ4 ~ 4.5x 3. Carrot ZSTD ~ 4.9x
 * <p>
 * -- Ads performance Redis 7.2.4 = 1,527,734,024 Carrot no compression = 652,325,376 Carrot LZ4
 * compression = 476,455,424 Carrot ZSTD compression = 364,447,872 Memory ratio Redis/Carrot: 1.
 * Carrot no compression ~ 2.34x 2. Carrot LZ4 ~ 3.2x 3. Carrot ZSTD ~ 4.2x
 * <p>
...
public class TestCarrotAdServerAdIndexing {
  ...
  private static void doWordAds(BigSortedMap map) {
    ...
    for (int i = 0; i < MAX_WORDS; i++) {
      ...
      for (int j = 0; j < n; j++) {
        ...
        int mSize = Utils.SIZEOF_INT;
        UnsafeAccess.putInt(mPtr, id);
        ZSets.ZADD(map, keyPtr, keySize, new double[] { 0 }, new long[] { mPtr },
          new int[] { mSize }, true);
        UnsafeAccess.free(mPtr);
        count++;
        if (count % 100000 == 0) {
  ...
  private static void doAdeCPM(BigSortedMap map) {
    ...
    for (int i = 0; i < MAX_ADS; i++) {
      ...
      ZSets.ZADD(map, keyPtr, keySize, new double[] { eCPM }, new long[] { mPtr },
        new int[] { mSize }, true);
      UnsafeAccess.free(mPtr);
    }
    UnsafeAccess.free(keyPtr);
  ...
  private static void doAdBase(BigSortedMap map) {
    ...
    for (int i = 0; i < MAX_ADS; i++) {
      ...
      ZSets.ZADD(map, keyPtr, keySize, new double[] { base }, new long[] { mPtr },
        new int[] { mSize }, true);
      UnsafeAccess.free(mPtr);
    }
    UnsafeAccess.free(keyPtr);
