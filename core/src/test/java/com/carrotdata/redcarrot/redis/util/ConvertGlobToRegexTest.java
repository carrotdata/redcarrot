...
public class ConvertGlobToRegexTest {
  ...
  public void testRedisGlob() {
    String[] redis = new String[] { "h?llo", "h*llo", "h**llo", "h[ae]llo", "h[^e]llo", "h[a-c]llo",
        "?[a-cA-C]*", "h[^e]llo", "h[^a-c]llo", "[^e]llo", "h*", "ha*", "[o]", "[^o]", "*", "?",
        "*?", "?*" };
    String[] input = new String[] { "hello", "hallo", "hbllo", "hdllo", "hublo", "a", "o", "oo",
        "hcllo", "cat", "bat", "Cat" };
    for (int i = 0; i < redis.length; i++) {
      String javaRegex = convertGlobToRegex(redis[i]);
      System.out.println();
      for (int j = 0; j < input.length; j++) {
        System.out.printf("redis=%s java=%s input=%s matches=%b\n", redis[i], javaRegex, input[j],
          input[j].matches(javaRegex));
      }
      System.out.println();
    }
