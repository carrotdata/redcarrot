 ...
 *
 * <p>KEY -> [e1,e2,e3,e4, ..., eN] we store them in a sequence of Key-Value pairs as following:
 *
 * <p>[KEY_SIZE]KEY0-> Value([e1,e2,e3,...,eX]) [KEY_SIZE]KEYeX+1->Value([eX+1,eX+2,...]) [KEY_SIZE]
 * prepends Key to avoid possible key collisions
 *
 * <p>Value format:
 *
 * <p>N - number of elements - 2 bytes { element size - VLE (variable length encoding) element data
 * } +N
 */
public class Sets {
  ...
  private static ThreadLocal<SetExists> setExists =
      new ThreadLocal<SetExists>() {
        @Override
  ...
  private static ThreadLocal<SetDelete> setDelete =
      new ThreadLocal<SetDelete>() {
        @Override
   ...
   * @return number of elements added
   */
  public static int SADD_NEW_ZADD(
...
//    }
   ...
   * For TESTING only
   *
   * <p>Returns if member is a member of the set stored at key. Return value Integer reply,
   ...
   * @return 1 if exists, 0 - otherwise
   */
  public static int SISMEMBER(
      BigSortedMap map, long keyPtr, int keySize, long elemPtr, int elemSize) {
    int kSize = buildKey(keyPtr, keySize, elemPtr, elemSize);
   ...
   */
  public static long SPOP(
      BigSortedMap map, long keyPtr, int keySize, long bufferPtr, int bufferSize, int count) {
  ...
  private static int bulkDelete(
      BigSortedMap map, final long keyPtr, final int keySize, final long bufferPtr) {
   ...
   *
   * <p>When a count argument is passed and is positive, the elements are returned as if every
   * selected element is removed from the set (like the extraction of numbers in the game of Bingo).
   * However elements are not removed from the Set. So basically: No repeated elements are returned.
   * If count is bigger than the number of elements inside the Set, the command will only return the
   ...
   */
  public static long SRANDMEMBER(
      BigSortedMap map, long keyPtr, int keySize, long bufferPtr, int bufferSize, int count) {
    Key k = getKey(keyPtr, keySize);
    boolean distinct = count > 0;
    if (!distinct) {
  ...
  private static long readByIndex(
      SetScanner scanner, long[] index, long bufferPtr, int bufferSize) {
   ...
   */
  public static List<String> SSCAN(
      BigSortedMap map,
      String key,
   ...
   * @return set scanner
   */
  public static SetScanner getScanner(
