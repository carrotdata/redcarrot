...
/**
 * Bit count operation. Count the number of set bits (population counting) in a string. By default
 * all the bytes contained in the string are examined. It is possible to specify the counting
 * operation only in an interval passing the additional arguments start and end. Like for the
 * GETRANGE command start and end can contain negative values in order to index bytes starting from
 * the end of the string, where -1 is the last byte, -2 is the penultimate, and so forth.
