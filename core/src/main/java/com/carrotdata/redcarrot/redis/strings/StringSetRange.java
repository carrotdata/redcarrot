...
/**
 * String SETRANGE operation. Overwrites part of the string stored at key, starting at the specified
 * offset, for the entire length of value. If the offset is larger than the current length of the
 * string at key, the string is padded with zero-bytes to make offset fit. Non-existing keys are
 * considered as empty strings, so this command will make sure it holds a string large enough to be
 * able to set value at offset. Note that the maximum offset that you can set is 229 -1 (536870911),
 * as Redis Strings are limited to 512 megabytes. If you need to grow beyond this size, you can use
 * multiple keys.
 */
