...
import static com.carrotdata.redcarrot.redis.util.Commons.canSplit;
...
import static com.carrotdata.redcarrot.redis.util.Commons.keySize;
...
import static com.carrotdata.redcarrot.redis.util.Commons.setNumElements;
...
/**
 * This read-modify-write mutation is executed atomically and isolated It adds new field - value to
 * a given Hash, defined by a Key
 */
public class HashSet extends Operation {
  ...
  private void insertNewKVandFieldValue(long fieldPtr, int fieldSize) {
    ...
    int totalKeySize =
        buildKey(keyAddress + KEY_SIZE + Utils.SIZEOF_BYTE, kSize, fieldPtr, fieldSize);
    long kPtr = keyArena.get();
