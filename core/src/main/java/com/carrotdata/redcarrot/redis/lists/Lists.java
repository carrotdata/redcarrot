 ...
 * <p>
 * Current limitation: Maximum list size is 2 ^31 ~ 2 billion elements
 */
public class Lists {
   ...
   * List format:
   * <p>
   * 1 byte - compression codec (0 for now, future) 4 bytes - list size (number of elements)
   * Segment+
   * <p>
   * Segment: Segment data (Segment header contains all needed information to deserialize it)
   * <p>
   ...
   * <p>
   * Available since 6.2.0. Time complexity: O(1) Atomically returns and removes the first/last
   ...
   */
  public static int LMOVE(BigSortedMap map, long srcKeyPtr, int srcKeySize, long dstKeyPtr,
      int dstKeySize, Side src, Side dst, long buffer, int bufferSize) {
   ...
   * <p>
   * Available since 6.2.0. Time complexity: O(1) BLMOVE is the blocking variant of LMOVE. When
   ...
   * <p>
   * Return value Bulk string reply: the element being popped from source and pushed to destination.
   * If timeout is reached, a Null reply is returned. Pattern: Reliable queue Please see the pattern
   * description in the LMOVE documentation. Pattern: Circular list Please see the pattern
   * description in the LMOVE documentation.
   * @param map sorted map storage
   * @param srcKeyPtr source list key address
   * @param srcKeySize source list key size
   * @param dstKeyPtr dst key list address
   ...
   * @return size of a value being moved or -1
   */
  public static long BLMOVE(BigSortedMap map, long srcKeyPtr, int srcKeySize, long dstKeyPtr,
      int dstKeySize, Side src, Side dst, long timeout, long buffer, int bufferSize) {
   ...
   * <p>
   * BLPOP list1 list2 list3 0 BLPOP guarantees to return an element from the list stored at list2
   ...
   * an LPUSH or RPUSH operation against one of the keys. Once new data is present on one of the
   * lists, the client returns with the name of the key unblocking it and the popped value. When
   * BLPOP causes a client to block and a non-zero timeout is specified, the client will unblock
   * returning a nil multi-bulk value when the specified timeout has expired without a push
   ...
   * command: Variadic push operations such as LPUSH mylist a b c. After an EXEC of a MULTI block
   * with multiple push operations against the same list.
   * <p>
   * Executing a Lua Script with Redis 2.6 or newer.
   * <p>
   * When multiple elements are pushed inside a list where there are clients blocking, the behavior
   * is different for Redis 2.4 and Redis 2.6 or newer. For Redis 2.6 what happens is that the
   * command performing multiple pushes is executed, and only after the execution of the command the
   * blocked clients are served. Consider this sequence of commands.
   * <p>
   * Client A: BLPOP foo 0 Client B: LPUSH foo a b c
   * <p>
   * If the above condition happens using a Redis 2.6 server or greater, Client A will be served
   ...
   * however this setup makes sense almost solely when it is the last command of the pipeline. Using
   * BLPOP inside a MULTI / EXEC block does not make a lot of sense as it would require blocking the
   * entire server in order to execute the block atomically, which in turn does not allow other
   * clients to perform a push operation. For this reason the behavior of BLPOP inside MULTI / EXEC
   * when the list is empty is to return a nil multi-bulk reply, which is the same thing that
   * happens when the timeout is reached. If you like science fiction, think of time flowing at
   * infinite speed inside a MULTI / EXEC block...
   * <p>
   * Return value Array reply: specifically: A nil multi-bulk when no element could be popped and
   ...
   *         Buffer format:
   *         <p>
   *         Key index = 4 bytes Value
   */
  public static long BLPOP(BigSortedMap map, long[] keyPtrs, int[] keySizes, long buffer,
      int bufferSize) {
    // Looks thread-safe
    int size;
    for (int i = 0; i < keyPtrs.length; i++) {
      size = (int) LPOP(map, keyPtrs[i], keySizes[i], buffer + Utils.SIZEOF_INT,
   ...
   * <p>
   * A nil multi-bulk when no element could be popped and the timeout expired. A two-element
   ...
   *         Key index = 4 bytes Value
   */
  public static long BRPOP(BigSortedMap map, long[] keyPtrs, int[] keySizes, long buffer,
      int bufferSize) {
    // Looks thread - safe
    int size;
    for (int i = 0; i < keyPtrs.length; i++) {
      size = (int) RPOP(map, keyPtrs[i], keySizes[i], buffer + Utils.SIZEOF_INT,
        bufferSize - Utils.SIZEOF_INT);
      if (size + Utils.SIZEOF_INT > bufferSize) {
        return size + Utils.SIZEOF_INT;
      } else if (size > 0) {
   ...
   * <p>
   * Pattern: Reliable queue Please see the pattern description in the RPOPLPUSH documentation.
   * <p>
   * Pattern: Circular list Please see the pattern description in the RPOPLPUSH documentation.
   * @param map sorted map storage
  ...
  public static long BRPOPLPUSH(BigSortedMap map, long srcKeyPtr, int srcKeySize, long dstKeyPtr,
    ...
    return BLMOVE(map, srcKeyPtr, srcKeySize, dstKeyPtr, dstKeySize, Side.RIGHT, Side.LEFT, timeout,
      buffer, bufferSize);
  }
   ...
   * Return value Bulk string reply: the element being popped and pushed.
   * <p>
   * Examples redis> RPUSH mylist "one" (integer) 1 redis> RPUSH mylist "two" (integer) 2 redis>
   ...
   * of an N-elements list, one after the other, in O(N) without transferring the full list from the
   * server to the client using a single LRANGE operation. The above pattern works even if the
   * following two conditions: There are multiple clients rotating the list: they'll fetch different
   * elements, until all the elements of the list are visited, and the process restarts. Even if
   * other clients are actively pushing new items at the end of the list. The above makes it very
   * simple to implement a system where a set of items must be processed by N workers continuously
   * as fast as possible. An example is a monitoring system that must check that a set of web sites
   * are reachable, with the smallest delay possible, using a number of parallel workers. Note that
   * this implementation of workers is trivially scalable and reliable, because even if a message is
  ...
  public static int RPOPLPUSH(BigSortedMap map, long srcKeyPtr, int srcKeySize, long dstKeyPtr,
    ...
    return LMOVE(map, srcKeyPtr, srcKeySize, dstKeyPtr, dstKeySize, Side.RIGHT, Side.LEFT, buffer,
      bufferSize);
  }
   ...
   */
  public static int LINDEX(BigSortedMap map, long keyPtr, int keySize, long index, long buffer,
      int bufferSize) {
   ...
   * <p>
   * Available since 2.2.0. Time complexity: O(N) where N is the number of elements to traverse
   ...
   * <p>
   * Return value Integer reply: the length of the list after the insert operation, or -1 when the
   * value pivot was not found.
   * @param map sorted map storage
   * @param keyPtr list key pointer
   * @param keySize list key size
   ...
   * <p>
   * LPOP key Available since 1.0.0. Time complexity: O(1) Removes and returns the first element of
   * the list stored at key. Return value Bulk string reply: the value of the first element, or nil
   ...
   * the "rank" of the first element to return, in case there are multiple matches. A rank of 1
   * means to return the first match, 2 to return the second match, and so forth. For instance, in
   * the above example the element "c" is present multiple times, if I want the index of the second
   * match, I'll write:
   * <p>
   * > LPOS mylist c RANK 2 6
   * <p>
   * That is, the second occurrence of "c" is at position 6. A negative "rank" as the RANK argument
   * tells LPOS to invert the search direction, starting from the tail to the head. So, we want to
   * say, give me the first element starting from the tail of the list:
   * <p>
   * > LPOS mylist c RANK -1 7
   * <p>
   * Note that the indexes are still reported in the "natural" way, that is, considering the first
   * element starting from the head of the list at index 0, the next element at index 1, and so
   * forth. This basically means that the returned indexes are stable whatever the rank is positive
   * or negative. Sometimes we want to return not just the Nth matching element, but the position of
   * all the first N matching elements. This can be achieved using the COUNT option.
   * <p>
   * > LPOS mylist c COUNT 2 [2,6]
   * <p>
   * We can combine COUNT and RANK, so that COUNT will try to return up to the specified number of
   * matches, but starting from the Nth match, as specified by the RANK option.
   * <p>
   * > LPOS mylist c RANK -1 COUNT 2 [7,6]
   * <p>
   * When COUNT is used, it is possible to specify 0 as the number of matches, as a way to tell the
   * command we want all the matches found returned as an array of indexes. This is better than
   * giving a very large COUNT option because it is more general.
   * <p>
   * > LPOS mylist c COUNT 0 [2,6,7]
   * <p>
   * When COUNT is used and no match is found, an empty array is returned. However when COUNT is not
   * used and there are no matches, the command returns NULL. Finally, the MAXLEN option tells the
   * command to compare the provided element only with a given maximum number of list items. So for
   * instance specifying MAXLEN 1000 will make sure that the command performs only 1000 comparisons,
   * effectively running the algorithm on a subset of the list (the first part or the last part
   * depending on the fact we use a positive or negative rank). This is useful to limit the maximum
   * complexity of the command. It is also useful when we expect the match to be found very early,
   * but want to be sure that in case this is not true, the command does not take too much time to
   * run.
   * <p>
   * Return value
   * <p>
   * The command returns the integer representing the matching element, or null if there is no
   ...
   */
  public static int LPOS(BigSortedMap map, long keyPtr, int keySize, long elemPtr, int elemSize,
      int rank, int numMatches, int maxlen, long buffer, int bufferSize) {
    // TODO
    return 0;
  }
   ...
   * <p>
   * Available since 1.0.0. Time complexity: O(1) for each element added, so O(N) to add N elements
   * when the command is called with multiple arguments. Insert all the specified values at the head
   * of the list stored at key. If key does not exist, it is created as empty list before performing
   * the push operations. When key holds a value that is not a list, an error is returned. It is
   * possible to push multiple elements using a single command call just specifying multiple
   ...
   * as third element.
   * <p>
   * Return value Integer reply: the length of the list after the push operations. History >= 2.4:
   ...
   */
  public static long LPUSH(BigSortedMap map, long keyPtr, int keySize, long[] elemPtrs,
      int[] elemSizes) {
   ...
   * <p>
   * Available since 2.2.0. Time complexity: O(1) for each element added, so O(N) to add N elements
   * when the command is called with multiple arguments. Inserts specified values at the head of the
   * list stored at key, only if key already exists and holds a list. In contrary to LPUSH, no
   * operation will be performed when key does not yet exist. Return value Integer reply: the length
   * of the list after the push operation. History >= 4.0: Accepts multiple element arguments. In
   * Redis versions older than 4.0 it was possible to push a single value per command.
   * @param map sorted map storage
   * @param keyPtr list key pointer
   * @param keySize list key size
   * @param elemPtrs array of element pointers
   ...
   */
  public static long LPUSHX(BigSortedMap map, long keyPtr, int keySize, long[] elemPtrs,
      int[] elemSizes) {
    // TODO: implement as an Operation
    Key key = getKey(keyPtr, keySize);
    try {
  ...
  private static Segment firstSegment(BigSortedMap map, long kPtr, int kSize, long valueBuf,
      int valueBufSize) {
    Segment s = segment.get();
    long size = map.get(kPtr, kSize, valueBuf, valueBufSize, 0);
    if (size < 0) {
  ...
  private static Segment lastSegment(BigSortedMap map, long kPtr, int kSize, long valueBuf,
      int valueBufSize) {
    Segment s = segment.get();
    long size = map.get(kPtr, kSize, valueBuf, valueBufSize, 0);
    if (size < 0) {
   ...
   * <p>
   * Out of range indexes will not produce an error. If start is larger than the end of the list, an
   * empty list is returned. If stop is larger than the actual end of the list, Redis will treat it
   * like the last element of the list.
   * <p>
   * -inf - no limit for start +inf - no limit for end
   * <p>
   ...
   * <p>
   * Available since 1.0.0. Time complexity: O(N+M) where N is the length of the list and M is the
   * number of elements removed. Removes the first count occurrences of elements equal to element
   * from the list stored at key. The count argument influences the operation in the following ways:
   * count > 0: Remove elements equal to element moving from head to tail. count < 0: Remove
   * elements equal to element moving from tail to head. count = 0: Remove all elements equal to
   * element. For example, LREM list -2 "hello" will remove the last two occurrences of "hello" in
   * the list stored at list. Note that non-existing keys are treated like empty lists, so when key
   ...
   */
  public static long LREM(BigSortedMap map, long keyPtr, int keySize, int count, long elemPtr,
      int elemSize) {
    // TODO: Operation
    Key key = getKey(keyPtr, keySize);
    ...
    try {
      ...
      while (s != null) {
        ...
        s = reverse ? updateSegmentInChainReverse(map, s, valueBuf)
   ...
   */
  public static long LSET(BigSortedMap map, long keyPtr, int keySize, long index, long elemPtr,
      int elemSize) {
   ...
   * <p>
   * Available since 1.0.0. Time complexity: O(N) where N is the number of elements to be removed by
   * the operation. Trim an existing list so that it will contain only the specified range of
   ...
   * RPOP key
   * <p>
   * Available since 1.0.0. Time complexity: O(1) Removes and returns the last element of the list
   * stored at key. Return value Bulk string reply: the value of the last element, or nil when key
   * does not exist.
   * @param map sorted map storage
   * @param keyPtr list key pointer
   * @param keySize list key size
   ...
   * <p>
   * Available since 1.0.0. Time complexity: O(1) for each element added, so O(N) to add N elements
   * when the command is called with multiple arguments. Insert all the specified values at the tail
   * of the list stored at key. If key does not exist, it is created as empty list before performing
   * the push operation. When key holds a value that is not a list, an error is returned. It is
   * possible to push multiple elements using a single command call just specifying multiple
   ...
   * as third element.
   * <p>
   * Return value Integer reply: the length of the list after the push operation. History >= 2.4:
   * Accepts multiple element arguments. In Redis versions older than 2.4 it was possible to push a
   * single value per command.
   * @param map sorted map storage
   * @param keyPtr list key pointer
   ...
   */
  public static long RPUSH(BigSortedMap map, long keyPtr, int keySize, long[] elemPtrs,
      int[] elemSizes) {
    Key key = getKey(keyPtr, keySize);
    try {
      KeysLocker.writeLock(key);
   ...
   * <p>
   * Available since 2.2.0. Time complexity: O(1) for each element added, so O(N) to add N elements
   * when the command is called with multiple arguments. Inserts specified values at the tail of the
   * list stored at key, only if key already exists and holds a list. In contrary to RPUSH, no
   ...
   * <p>
   * History >= 4.0: Accepts multiple element arguments. In Redis versions older than 4.0 it was
   ...
   */
  public static long RPUSHX(BigSortedMap map, long keyPtr, int keySize, long[] elemPtrs,
      int[] elemSizes) {
    Key key = getKey(keyPtr, keySize);
    try {
      KeysLocker.writeLock(key);
