...
public final class IndexBlock implements Comparable<IndexBlock> {
  ...
  public static final double MAX_MERGE_RATIO = 0.25d;

  public static final String MAX_BLOCK_SIZE_KEY = "max.index.block.size";
  /*
   * TODO: check usage for DataBlocks
   */
  public static int MAX_BLOCK_SIZE = 4096;

  public static final int DATA_BLOCK_STATIC_PREFIX = 20;
  ...
  private final int keyLength(long blockAddress) {
    if (isExternalBlock(blockAddress)) {
      long recAddress =
          UnsafeAccess.toLong(blockAddress + DATA_BLOCK_STATIC_PREFIX + KEY_SIZE_LENGTH);
...
//    lock.writeLock().unlock();
  ...
  boolean tryUpdateFirstKey(DataBlock block) {
    ...
    if (mustAllocateExternally(keyLength)) {
      required = DATA_BLOCK_STATIC_OVERHEAD + KEY_SIZE_LENGTH + ADDRESS_SIZE - recLength;
   ...
   * @return true/false
   */
  boolean isLessThanMin(long keyPtr, int keyLength, long version) throws RetryOperationException {
  ...
  private void deleteBlock(DataBlock b, boolean extAllocs) {
    ...
    int toMove = DATA_BLOCK_STATIC_OVERHEAD + KEY_SIZE_LENGTH + blockKeyLength;
    if (mustAllocateExternally(keyLength)) {
