 ...
 *
 * <p>The command returns the position of the first bit set to 1 or 0 according to the request. If
 * we look for set bits (the bit argument is 1) and the string is empty or composed of just zero
 * bytes, -1 is returned. If we look for clear bits (the bit argument is 0) and the string only
 * contains bit set to 1, the function returns the first bit not part of the string on the right. So
 * if the string is three bytes set to the value 0xff the command BITPOS key 0 will return 24, since
 * up to bit 23 all the bits are 1. Basically, the function considers the right of the string as
 * padded with zeros if you look for clear bits and specify no range or the start argument only.
 * However, this behavior changes if you are looking for clear bits and specify a range with both
 * start and end. If no clear bit is found in the specified range, the function returns -1 as the
...
public class StringBitPos extends Operation {
  ...
  public boolean execute() {
    ...
    this.position =
        bit == 1
            ? Utils.bitposSet(valuePtr + start, (int) (end - start) + 1)
