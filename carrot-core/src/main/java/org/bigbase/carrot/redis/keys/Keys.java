...
public class Keys {
  ...
  /**
   * Available since 1.0.0. Time complexity: O(1) Returns the remaining time to live of a key that
   * has a timeout. This introspection capability allows a Redis client to check how many seconds a
   * given key will continue to be part of the dataset. In Redis 2.6 or older the command returns -1
   * if the key does not exist or if the key exist but has no associated expire. Starting with Redis
   * 2.8 the return value in case of error changed: The command returns -2 if the key does not
  ...
  /**
   * Available since 2.6.0. Time complexity: O(1) Like TTL this command returns the remaining time
   * to live of a key that has an expire set, with the sole difference that TTL returns the amount
   * of remaining time in seconds while PTTL returns it in milliseconds. In Redis 2.6 or older the
   * command returns -1 if the key does not exist or if the key exist but has no associated expire.
   * Starting with Redis 2.8 the return value in case of error changed: The command returns -2 if
   * the key does not exist. The command returns -1 if the key exists but has no associated expire.
   * Return value Integer reply: TTL in milliseconds, or a negative value in order to signal an
  ...
  /**
   * DEL key [key ...] Available since 1.0.0. Time complexity: O(N) where N is the number of keys
   * that will be removed. When a key to remove holds a value other than a string, the individual
   * complexity for this key is O(M) where M is the number of elements in the list, set, sorted set
   ...
   * This means that all the operations that conceptually alter the value stored at the key without
   * replacing it with a new one will leave the timeout untouched. For instance, incrementing the
   * value of a key with INCR, pushing a new value into a list with LPUSH, or altering the field
   * value of a hash with HSET are all operations that will leave the timeout untouched. The timeout
   * can also be cleared, turning the key back into a persistent key, using the PERSIST command. If
   * a key is renamed with RENAME, the associated time to live is transferred to the new key name.
