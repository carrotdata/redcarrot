...
public class Server {
  ...
  /**
   * The TIME command returns the current server time as a two items lists: a Unix timestamp and the
   ...
   * the server). Conditions where a SHUTDOWN fails When the Append Only File is enabled the
   * shutdown may fail because the system is in a state that does not allow to safely immediately
   * persist on disk. Normally if there is an AOF child process performing an AOF rewrite, Redis
   * will simply kill it and exit. However there are two conditions where it is unsafe to do so, and
   * the SHUTDOWN command will be refused with an error instead. This happens when: The user just
   * turned on AOF, and the server triggered the first AOF rewrite in order to create the initial
   * AOF file. In this context, stopping will result in losing the dataset at all: once restarted,
   * the server will potentially have AOF enabled without having any AOF file at all. A replica with
   * AOF enabled, reconnected with its master, performed a full resynchronization, and restarted the
   * AOF file, triggering the initial AOF creation process. In this case not completing the AOF
   * rewrite is dangerous because the latest dataset received from the master would be lost. The new
   * master can actually be even a different instance (if the REPLICAOF or SLAVEOF command was used
   * in order to reconfigure the replica), so it is important to finish the AOF rewrite and start
   * with the correct data set representing the data set in memory when the server was terminated.
   * There are conditions when we want just to terminate a Redis instance ASAP, regardless of what
   * its content is. In such a case, the right combination of commands is to send a CONFIG
  ...
  /**
   * Save the DB in background. Normally the OK code is immediately returned. Redis forks, the
   * parent continues to serve the clients, the child saves the DB on disk then exits. An error is
   * returned if there is already a background save running or if there is another
   * non-background-save process running, specifically an in-progress AOF rewrite. If BGSAVE
   * SCHEDULE is used, the command will immediately return OK when an AOF rewrite is in progress and
   * schedule the background save to run at the next opportunity. A client may be able to check if
   * the operation succeeded using the LASTSAVE command. Please refer to the persistence
  ...
  /**
   * FLUSHALL [ASYNC|SYNC] Available since 1.0.0. Time complexity: O(N) where N is the total number
   * of keys in all databases Delete all the keys of all the existing databases, not just the
   * currently selected one. This command never fails. By default, FLUSHALL will synchronously flush
   * all the databases. Starting with Redis 6.2, setting the lazyfree-lazy-user-flush configuration
   * directive to "yes" changes the default flush mode to asynchronous. It is possible to use one of
   * the following modifiers to dictate the flushing mode explicitly: ASYNC: flushes the databases
